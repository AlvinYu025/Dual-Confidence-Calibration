...
parser.add_argument('--proportion', type=float, default=0.17, help="Proportion of parameters to be frozen")
...
def main(args, model_name, trainloader, testloader):
    ...
    best_model, best_acc, final_model, final_acc = engine.train_with_tl(args, net, criterion, optimizer, lr_schedular, trainloader, testloader, n_epochs)

    # torch.save({'state_dict': best_model.state_dict()},
    #            os.path.join(model_path, "{}_tl_best_{:.2f}_{:.2f}.tar").format(model_name, proportion, best_acc[0]))
    
    torch.save({'state_dict': final_model.state_dict()},
               os.path.join(model_path, "{}_tl_final_{:.2f}_{:.2f}.tar").format(model_name, proportion, final_acc[0]))
...
if __name__ == '__main__':
    ...
    log_file = "tl_{}.txt".format(model_name)
