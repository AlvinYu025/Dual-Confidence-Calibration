...
def main(args, model_name, trainloader, testloader):
    ...
    best_model, best_acc, final_model, final_acc = engine.train_reg(args, net, criterion, optimizer, lr_schedular, trainloader, testloader, n_epochs)

    # torch.save({'state_dict': best_model.state_dict()},
    #            os.path.join(model_path, "{}_reg_best_{:.2f}.tar").format(model_name, best_acc[0]))

    torch.save({'state_dict': final_model.state_dict()},
               os.path.join(model_path, "{}_reg_530_final_{:.2f}.tar").format(model_name, final_acc[0]))
...
if __name__ == '__main__':
    ...
    log_file = "reg_{}.txt".format(model_name)
