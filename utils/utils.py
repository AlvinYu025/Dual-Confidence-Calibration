...
def init_dataloader(args, file_path, batch_size=64, mode="gan", iterator=False, ood=False):
    tf = time.time()
    ...
    data_set = dataloader.ImageFolder(args, file_path, mode, ood=ood)
...
def get_augmodel(model_name, nclass, path_T=None, dataset='celeba', defense="reg"):
    if model_name == "VGG16":
        if "bido" in defense:
            model = VGG16_bido(nclass)
        elif "vib" in defense:
            model = VGG16_vib(nclass)
        else:
            model = VGG16(nclass)
...
def get_GAN(dataset, gan_type, gan_model_dir, n_classes, z_dim, target_model, defense="reg"):
    G = Generator(z_dim)
    if gan_type == True:
    ...
    if gan_type == True:
        ...
        path = os.path.join(os.path.join(os.path.join(gan_model_dir, dataset), target_model), defense)
...
def get_attack_model(args, args_json, eval_mode=False, defense="reg"):
    ...
    G, D = get_GAN(args_json['dataset']['name'], gan_type=args.improved_flag,
                   ...
                   n_classes=n_classes, z_dim=100, target_model=model_types_[0], defense=defense)
    ...
    for i in range(len(cid)):
        ...
        model = get_augmodel(model_types_[id_], n_classes, checkpoints[id_], dataset, defense=defense)
        ...
        if args.loss == 'logit_loss':
            if model_types_[id_] == "IR152" or model_types_[id_] == "VGG16" or model_types_[id_] == "FaceNet64":
                # target model
                p_reg = os.path.join(args_json["dataset"]["p_reg_path"], '{}_{}_{}_p_reg.pt'.format(defense, dataset, model_types_[
                    id_]))  # './p_reg/{}_{}_p_reg.pt'.format(dataset,model_types_[id_])
            else:
                # aug model
                p_reg = os.path.join(args_json["dataset"]["p_reg_path"],
                                     '{}_{}_{}_{}_p_reg.pt'.format(defense, dataset, model_types_[0], model_types_[
...
def write_list_to_csv(filename, precision_list):
