...
def KED_inversion(G, D, T, E, iden, batch_size, num_candidates, lr=2e-2, momentum=0.9, lamda=100,
    ...
    for i in range(iter_times):
        ...
        if improved:
            Prior_Loss = torch.mean(F.softplus(utils.log_sum_exp(label))) - torch.mean(utils.log_sum_exp(label))
        else:
    ...
    for i in range(z_init.shape[0]):
        ...
        for i_episode in range(1, max_episodes + 1):
            ...
            for t in range(max_step):
                ...
                state_output = target_model(state_image)
                state_output = state_output[1]

                action_output = target_model(action_image)
            ...
            if i_episode % 1000 == 0:
                ...
                opt_output = target_model(opt_img)
                opt_output = opt_output[1]
