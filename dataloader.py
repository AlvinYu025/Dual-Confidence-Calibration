...
class ImageFolder(data.Dataset):
    def __init__(self, args, file_path, mode, ood=False):
        self.args = args
        self.mode = mode
        self.ood = ood
        if ood:
            self.img_path = args["OOD_dataset"]["img_path"]
            self.n_classes = args["OOD_dataset"]["n_classes"]
            self.model_name = args["OOD_dataset"]["model_name"]
            self.dataset_name = args["OOD_dataset"]["name"]
        else:
            if mode == 'gan':
        ...
        if self.mode is not "gan":
            if ood:
                print("Load " + str(self.num_img) + " OOD images")
            else:
    ...
    def get_processor(self):
        ...
        if self.mode == "train":
            ...
            proc.append(transforms.ToTensor())
            if self.mode == 'test' or self.mode == 'train' or self.dataset_name != 'facescrub':
                proc.append(transforms.Lambda(crop))
            proc.append(transforms.ToPILImage())
            proc.append(transforms.Resize((re_size, re_size)))
            proc.append(transforms.ToTensor())
